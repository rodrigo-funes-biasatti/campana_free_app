...
<div class="row justify-content-center">
	...
	<div class="col-4">
		...
		<br>
		<caption><strong>Descripción:</strong> La siguiente tabla muestra todas las asistencias del evento seleccionado.</caption>
		<br>
		<caption><strong>EVENTO:</strong> {{$evento->nombre}}</caption>
	</div>
</div>
...
<div style="text-align: center; margin-bottom: 30px;">
	<form action="{{route('descRepGen')}}" method="POST">
...
<script>
	...
	var myChart = new Chart(ctx, {
		type: 'bar',
		data: {
			labels: ["Entradas de {{$evento->nombre}}"],
			datasets: [{
				label: 'Normal',
				data: [{{$normal}}],
				backgroundColor: [
		...
		options: {
			...
			title:{
				display: true,
				text: 'Cantidad de entredas: {{$vip+$normal}}'
			},
			responsive:true,
			legend:{
				position: 'top'
